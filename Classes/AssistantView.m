...
- (void)resetLiblinphone:(BOOL)core {
	...
	if (core) {
		[VirtuPhoneManager.instance resetLinphoneCore];
	}
	account_creator = linphone_account_creator_new(
		LC, [VirtuPhoneManager.instance lpConfigStringForKey:@"xmlrpc_url" inSection:@"assistant" withDefault:@""]
				.UTF8String);
...
- (void)loadAssistantConfig:(NSString *)rcFilename {
	linphone_core_load_config_from_xml(LC,
									   [VirtuPhoneManager bundleFile:rcFilename].UTF8String);
	if (account_creator) {
		...
		linphone_account_creator_set_domain(account_creator, [[VirtuPhoneManager.instance lpConfigStringForKey:@"domain" inSection:@"assistant" withDefault:@""] UTF8String]);
		linphone_account_creator_set_algorithm(account_creator, [[VirtuPhoneManager.instance lpConfigStringForKey:@"algorithm" inSection:@"assistant" withDefault:@""] UTF8String]);
	}
	[self changeView:nextView back:FALSE animation:TRUE];
}

- (void)reset {
	[VirtuPhoneManager.instance removeAllAccounts];
	[self resetTextFields];
	[self changeView:_welcomeView back:FALSE animation:FALSE];
...
- (void)configureProxyConfig {
	...
	if (new_config) {
		...
		[[VirtuPhoneManager.instance fastAddressBook] fetchContactsInBackGroundThread];
	} else
		[self displayAssistantConfigurationError];

	[VirtuPhoneManager.instance migrationPerAccount];
}
...
- (void)changeView:(UIView *)view back:(BOOL)back animation:(BOOL)animation {
	...
	if (view == _welcomeView) {
		BOOL show_logo = [VirtuPhoneManager.instance lpConfigBoolForKey:@"show_assistant_logo_in_choice_view_preference"];
		BOOL show_extern = ![VirtuPhoneManager.instance lpConfigBoolForKey:@"hide_assistant_custom_account"];
		BOOL show_new = ![VirtuPhoneManager.instance lpConfigBoolForKey:@"hide_assistant_create_account"];
		BOOL show_fetch_remote = ![VirtuPhoneManager.instance lpConfigBoolForKey:@"show_remote_provisioning_in_assistant"];
    ...
    if (currentView == _qrCodeView) {
        ...
        VirtuPhoneAppDelegate *delegate = (VirtuPhoneAppDelegate *)UIApplication.sharedApplication.delegate;
		...
		case LinphoneRegistrationOk: {
			...
			[VirtuPhoneManager.instance
				lpConfigSetInt:[NSDate new].timeIntervalSince1970 +
							   [VirtuPhoneManager.instance lpConfigIntForKey:@"link_account_popup_time" withDefault:84200]
...
- (void)configuringUpdate:(NSNotification *)notif {
	...
	switch (status) {
		case LinphoneConfiguringSuccessful:
			...
			[VirtuPhoneManager.instance lpConfigSetInt:[NSDate new].timeIntervalSince1970
											  forKey:@"must_link_account_time"];
...
void assistant_is_account_linked(LinphoneAccountCreator *creator, LinphoneAccountCreatorStatus status,
	...
	if (status == LinphoneAccountCreatorStatusAccountLinked) {
		...
		[VirtuPhoneManager.instance lpConfigSetInt:[NSDate new].timeIntervalSince1970 forKey:@"must_link_account_time"];
	} else {
		[thiz showErrorPopup:resp];
	}
}
...
- (IBAction)onGotoRemoteProvisioningClick:(id)sender {
    ONCLICKBUTTON(sender, 100, {
        ...
        [VirtuPhoneManager.instance lpConfigStringForKey:@"config-uri" inSection:@"misc"];
...
- (IBAction)onLoginClick:(id)sender {
	ONCLICKBUTTON(sender, 100, {
		...
		if (config) {
			[[VirtuPhoneManager instance] configurePushTokenForProxyConfig:config];
...
- (IBAction)onRemoteProvisioningLoginClick:(id)sender {
	ONCLICKBUTTON(sender, 100, {
        _waitView.hidden = NO;
        [VirtuPhoneManager.instance lpConfigSetInt:1 forKey:@"transient_provisioning" inSection:@"misc"];
        [self configureProxyConfig];
    });
}
