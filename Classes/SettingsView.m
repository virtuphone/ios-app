...
- (void)viewDidLoad {
	...
	settingsStore = [[VirtuPhoneCoreSettingsStore alloc] init];
...
- (void)appSettingChanged:(NSNotification *)notif {
	...
	if ([@"enable_video_preference" compare:notif.object] == NSOrderedSame) {
		...
		[VirtuPhoneManager.instance lpConfigSetInt:debugLevel forKey:@"debugenable_preference"];
        ...
        if (removeFromHiddenKeys)
            [VirtuPhoneManager.instance lpConfigSetInt:10000000 forKey:@"auto_download_incoming_files_max_size"];
...
- (NSSet *)findHiddenKeys {
	...
	int debugLevel = [VirtuPhoneManager.instance lpConfigIntForKey:@"debugenable_preference"];
	BOOL debugEnabled = (debugLevel >= ORTP_DEBUG && debugLevel < ORTP_ERROR);
	if (!debugEnabled) {
	...
	if (![VirtuPhoneManager isCodecSupported:"h264"]) {
		[hiddenKeys addObject:@"h264_preference"];
	}
	if (![VirtuPhoneManager isCodecSupported:"mp4v-es"]) {
	...
	[hiddenKeys addObjectsFromArray:[[VirtuPhoneManager unsupportedCodecs] allObjects]];
	...
	if (![[VirtuPhoneManager.instance iapManager] enabled]) {
...
- (void)settingsViewController:(IASKAppSettingsViewController *)sender
	...
	if ([key isEqual:@"release_button"]) {
		...
		[VirtuPhoneManager.instance destroyLinphoneCore];
		[VirtuPhoneManager instanceRelease];
		...
		while (rooms) {
			...
			while (events) {
				...
				if (!linphone_chat_message_is_outgoing(msg)) {
					[VirtuPhoneManager setValueInMessageAppData:nil forKey:@"localimage" inMessage:msg];
					[VirtuPhoneManager setValueInMessageAppData:nil forKey:@"uploadQuality" inMessage:msg];
                    [VirtuPhoneManager setValueInMessageAppData:nil forKey:@"localvideo" inMessage:msg];
                    [VirtuPhoneManager setValueInMessageAppData:nil forKey:@"localfile" inMessage:msg];
				}
				events = events->next;
			}
			rooms = rooms->next;
	...
	if ([key isEqual:@"assistant_button"]) {
		...
		if ([VirtuPhoneManager.instance lpConfigBoolForKey:@"send_logs_include_linphonerc_and_chathistory"]) {
...
- (void)sendEmailWithDebugAttachments {
	...
	if ([VirtuPhoneManager.instance lpConfigBoolForKey:@"send_logs_include_linphonerc_and_chathistory"]) {
		...
		[attachments
			addObject:@[ [VirtuPhoneManager preferenceFile:@"linphonerc"], @"text/plain", @"linphone-configuration.rc" ]];

		// retrieve historydb
		[attachments addObject:@[
			[VirtuPhoneManager dataFile:@"linphone_chats.db"],
...
- (void)emailAttachments:(NSArray *)attachments {
	...
	if (error != nil) {
		...
		[picker setToRecipients:[NSArray
									arrayWithObjects:[VirtuPhoneManager.instance lpConfigStringForKey:@"debug_popup_email"
																						withDefault:@""],
													 nil]];
