...
+ (BOOL)hasSelfAvatar {
	return [VirtuPhoneManager.instance lpConfigStringForKey:@"avatar"] != nil;
}
+ (UIImage *)selfAvatar {
    return [VirtuPhoneManager.instance avatar];
}
...
+ (void)setDisplayNameLabel:(UILabel *)label forAddress:(const LinphoneAddress *)addr withAddressLabel:(UILabel*)addressLabel{
	...
	if (contact) {
		...
		label.text = [FastAddressBook displayNameForAddress:addr];
		if([VirtuPhoneManager.instance lpConfigBoolForKey:@"display_phone_only" inSection:@"app"])
