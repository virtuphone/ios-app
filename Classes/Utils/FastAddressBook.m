...
+ (UIImage *)imageForAddress:(const LinphoneAddress *)addr {
	if ([VirtuPhoneManager isMyself:addr] && [LinphoneUtils hasSelfAvatar]) {
...
+ (Contact *)getContact:(NSString *)address {
	if (VirtuPhoneManager.instance.fastAddressBook != nil) {
		@synchronized(VirtuPhoneManager.instance.fastAddressBook.addressBookMap) {
			return [VirtuPhoneManager.instance.fastAddressBook.addressBookMap objectForKey:address];
		}
	}
  	return nil;
}
...
+ (BOOL)isSipAddress:(CNLabeledValue<CNInstantMessageAddress *> *)sipAddr {
	...
	if ([service caseInsensitiveCompare:VirtuPhoneManager.instance.contactSipField] == NSOrderedSame)
		return TRUE;
...
+ (BOOL)contactHasValidSipDomain:(Contact *)contact {
	...
	NSString *domain = VirtuPhoneManager.instance.contactFilter;
...
-(void)updateFriend:(Contact*) contact{
	...
	BOOL enabled = [VirtuPhoneManager.instance lpConfigBoolForKey:@"use_rls_presence"];
	const MSList *lists = linphone_core_get_friends_lists(LC);
	while (lists) {
