...
+ (FileTransferDelegate *)messageDelegate:(LinphoneChatMessage *)message {
	for (FileTransferDelegate *ftd in [VirtuPhoneManager.instance fileTransferDelegates]) {
	...
	if (size == 0) {
        ...
        void (^block)(void)= ^ {
            if ([fileType isEqualToString:@"image"]) {
                ...
                [[VirtuPhoneManager.instance fileTransferDelegates] removeObject:thiz];
                ...
                [VirtuPhoneManager setValueInMessageAppData:@"saving..." forKey:@"localimage" inMessage:message];
                ...
                [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if (error) {
                            ...
                            [VirtuPhoneManager setValueInMessageAppData:nil forKey:@"localimage" inMessage:message];
                            ...
                            [VirtuPhoneManager setValueInMessageAppData:[placeHolder localIdentifier]
                                                               forKey:@"localimage"
                                                            inMessage:message];
                        }
                ...
                NSString *filePath = [[VirtuPhoneManager cacheDirectory] stringByAppendingPathComponent:name];
                [[NSFileManager defaultManager] createFileAtPath:filePath
                                                        contents:thiz.data
                                                      attributes:nil];
                // until image is properly saved, keep a reminder on it so that the
                // chat bubble is aware of the fact that image is being saved to device
                [VirtuPhoneManager setValueInMessageAppData:@"saving..." forKey:@"localvideo" inMessage:message];
                ...
                [[PHPhotoLibrary sharedPhotoLibrary] performChanges:^{
                    ...
                    dispatch_async(dispatch_get_main_queue(), ^{
                        if (error) {
                            ...
                            [VirtuPhoneManager setValueInMessageAppData:nil forKey:@"localvideo" inMessage:message];
                            UIAlertController *errView = [UIAlertController alertControllerWithTitle:NSLocalizedString(@"Transfer error", nil)
        ...
        if ([fileType isEqualToString:@"image"] || [fileType isEqualToString:@"video"]) {
            ...
            NSString *name =[NSString stringWithUTF8String:linphone_content_get_name(content)];
            [VirtuPhoneManager setValueInMessageAppData:@"saving..." forKey:key inMessage:message];

            //write file to path
            dispatch_async(dispatch_get_main_queue(), ^{
                if([view writeFileInICloud:thiz.data fileURL:[view getICloudFileUrl:name]]) {
                    [VirtuPhoneManager setValueInMessageAppData:name forKey:key inMessage:message];
...
- (void)uploadData:(NSData *)data  forChatRoom:(LinphoneChatRoom *)chatRoom type:(NSString *)type subtype:(NSString *)subtype name:(NSString *)name key:(NSString *)key keyData:(NSString *)keyData qualityData:(NSNumber *)qualityData {
    [VirtuPhoneManager.instance.fileTransferDelegates addObject:self];
    ...
    [VirtuPhoneManager setValueInMessageAppData:keyData forKey:key inMessage:_message];
    [VirtuPhoneManager setValueInMessageAppData:qualityData forKey:@"uploadQuality" inMessage:_message];
...
- (void)stopAndDestroy {
	[[VirtuPhoneManager.instance fileTransferDelegates] removeObject:self];
	if (_message != NULL) {
