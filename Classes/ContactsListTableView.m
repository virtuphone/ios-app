...
- (void)loadData {
	...
	addressBookMap = [VirtuPhoneManager.instance getVirtuPhoneManagerAddressBookMap];
	BOOL updated = [VirtuPhoneManager.instance getContactsUpdated];
	if(([previous isEqualToString:@"ContactsDetailsView"] && updated) || updated || [addressBookMap count] == 0){
		[VirtuPhoneManager.instance setContactsUpdated:FALSE];
		@synchronized(addressBookMap) {
			NSDictionary *allContacts = [[NSMutableDictionary alloc] initWithDictionary:VirtuPhoneManager.instance.fastAddressBook.addressBookMap];
			sortedAddresses = [[VirtuPhoneManager.instance.fastAddressBook.addressBookMap allKeys] sortedArrayUsingComparator:^NSComparisonResult(id a, id b) {
			...
			for (NSString *addr in sortedAddresses) {
				Contact *contact = nil;
				@synchronized(VirtuPhoneManager.instance.fastAddressBook.addressBookMap) {
					contact = [VirtuPhoneManager.instance.fastAddressBook.addressBookMap objectForKey:addr];
				}
				BOOL add = true;
		...
		[VirtuPhoneManager.instance setVirtuPhoneManagerAddressBookMap:addressBookMap];
	}
...
- (void)loadSearchedData {
	...
	@synchronized(addressBookMap) {
		...
		for (NSString *addr in sortedAddresses) {
                  @synchronized(
                      VirtuPhoneManager.instance.fastAddressBook.addressBookMap) {
                    Contact *contact =
                        [VirtuPhoneManager.instance.fastAddressBook.addressBookMap
                            objectForKey:addr];
	...
	if (editingStyle == UITableViewCellEditingStyleDelete) {
		...
		[UIConfirmationDialog ShowWithMessage:msg
					...
					onConfirmationClick:^() {
						...
						[[VirtuPhoneManager.instance fastAddressBook] deleteContact:contact];
