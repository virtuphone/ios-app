...
- (void)viewWillAppear:(BOOL)animated {
	...
	// Update on show
	VirtuPhoneManager *mgr = VirtuPhoneManager.instance;
...
- (void)viewDidLoad {
	...
	if (IPAD) {
		if (VirtuPhoneManager.instance.frontCamId != nil) {
...
- (void)viewDidAppear:(BOOL)animated {
	...
	[VirtuPhoneManager.instance shouldPresentLinkPopup];
}
	...
	if (![debugAddress isEqualToString:@""] && [address isEqualToString:debugAddress]) {
		...
		UIAlertAction* logAction = [UIAlertAction actionWithTitle:actionLog
														  ...
														  handler:^(UIAlertAction * action) {
																   ...
																   [VirtuPhoneManager.instance lpConfigSetInt:newDebugLevel forKey:@"debugenable_preference"];
		...
		UIAlertAction* remAction = [UIAlertAction actionWithTitle:NSLocalizedString(@"Remove account(s) and self destruct", nil)
															style:UIAlertActionStyleDefault
														  handler:^(UIAlertAction * action) {
															  linphone_core_clear_proxy_config([VirtuPhoneManager getLc]);
															  linphone_core_clear_all_auth_info([VirtuPhoneManager getLc]);
															  @try {
																  ...
																  if ([NSFileManager.defaultManager
																	   isDeletableFileAtPath:[VirtuPhoneManager preferenceFile:@"linphonerc"]] == YES) {
																	  [NSFileManager.defaultManager
																	   removeItemAtPath:[VirtuPhoneManager preferenceFile:@"linphonerc"]
																	   error:nil];
																  }
#ifdef DEBUG
