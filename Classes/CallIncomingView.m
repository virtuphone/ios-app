...
- (void)didRotateFromInterfaceOrientation:(UIInterfaceOrientation)fromInterfaceOrientation {
	...
	if (_earlyMedia && [VirtuPhoneManager.instance lpConfigBoolForKey:@"pref_accept_early_media"] && linphone_core_get_calls_nb(LC) < 2) {
...
- (void)callUpdate:(LinphoneCall *)acall state:(LinphoneCallState)astate {
	...
	} else if ([VirtuPhoneManager.instance lpConfigBoolForKey:@"auto_answer"]) {
...
- (void)setCall:(LinphoneCall *)call {
	_call = call;
	_earlyMedia = FALSE;
	if ([VirtuPhoneManager.instance lpConfigBoolForKey:@"pref_accept_early_media"] && linphone_core_get_calls_nb(LC) < 2) {
