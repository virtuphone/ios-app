...
- (instancetype)init {
	if ((self = [super init]) != nil) {
		_enabled = (([SKPaymentQueue canMakePayments]) &&
					([VirtuPhoneManager.instance lpConfigBoolForKey:@"enabled" inSection:@"in_app_purchase"]));
		_initialized = false;
		_available = false;
		_accountActivationInProgress = false;
		checkPeriod = [VirtuPhoneManager.instance lpConfigIntForKey:@"expiry_check_period" inSection:@"in_app_purchase"];
		warnBeforeExpiryPeriod = [VirtuPhoneManager.instance lpConfigIntForKey:@"warn_before_expiry_period" inSection:@"in_app_purchase"];
		lastCheck = 0;

		[XMLRPCHelper.self initArray];
		//========// for test only
		// int testExpiry = [VirtuPhoneManager.instance lpConfigIntForKey:@"expiry_time_test"
...
- (void)loadProducts {
	NSArray *list = [[[VirtuPhoneManager.instance lpConfigStringForKey:@"products_list" inSection:@"in_app_purchase"]
...
- (void)saveReceiptTemp:(NSString *)receipt {
	...
	[VirtuPhoneManager.instance lpConfigSetString:receipt forKey:@"save_tmp_receipt" inSection:@"in_app_purchase"];
}
...
- (void)removeTmpReceipt:(NSString *)receipt {
	...
	if ([VirtuPhoneManager.instance lpConfigStringForKey:@"save_tmp_receipt" inSection:@"in_app_purchase"])
		[VirtuPhoneManager.instance lpConfigSetString:@"0" forKey:@"save_tmp_receipt" inSection:@"in_app_purchase"];
}
...
- (NSString *)getTmpReceipt {
	...
	return [VirtuPhoneManager.instance lpConfigStringForKey:@"save_tmp_receipt" inSection:@"in_app_purchase"];
}
...
- (NSString *)getPassword {
	...
	if (cfg &&
		strcmp([VirtuPhoneManager.instance lpConfigStringForKey:@"domain_name"
													inSection:@"app"
	...
	return [self
		callXmlrpcRequestWithParams:@"update_expiration_date"
						  onSuccess:^(NSString *response) {
							if (response) {
								// NSString *productID = [VirtuPhoneManager.instance
