...
- (void)viewDidLoad {
	[super viewDidLoad];
	NSString *siteUrl =
		[[VirtuPhoneManager instance] lpConfigStringForKey:@"first_login_view_url"] ?: @"http://www.linphone.org";
...
- (void)viewWillAppear:(BOOL)animated {
	...
	// Update on show
	const MSList *list = linphone_core_get_proxy_config_list([VirtuPhoneManager getLc]);
	if (list != NULL) {
	...
	NSString *siteUrl =
		[[VirtuPhoneManager instance] lpConfigStringForKey:@"first_login_view_url"] ?: @"http://www.linphone.org";
	account_creator = linphone_account_creator_new([VirtuPhoneManager getLc], siteUrl.UTF8String);
...
- (void)configureStateUpdateEvent:(NSNotification *)notif {
	...
	switch (state) {
		case LinphoneConfiguringFailed: {
			...
			linphone_core_set_provisioning_uri([VirtuPhoneManager getLc], NULL);
			break;
		}
		default:
			break;
...
- (void)registrationUpdate:(LinphoneRegistrationState)state {
	switch (state) {
		case LinphoneRegistrationOk: {
			[[VirtuPhoneManager instance] lpConfigSetBool:FALSE forKey:@"enable_first_login_view_preference"];
			[_waitView setHidden:true];
