...
@implementation VirtuPhoneCoreSettingsStore
	...
	*maxPort = -1;
	...
	if ([matches count] == 1) {
		...
		if (!range) {
			...
			*minPort = [VirtuPhoneCoreSettingsStore validPort:[[text substringWithRange:rangeMinPort] intValue]];
			*maxPort = *minPort;
			return TRUE;
		} else {
			NSRange rangeMinPort = [match rangeAtIndex:1];
			...
			*maxPort = [VirtuPhoneCoreSettingsStore validPort:[[text substringWithRange:rangeMaxPort] intValue]];
			if (*minPort > *maxPort) {
				*minPort = *maxPort;
			}
			return TRUE;
...
- (void)transformCodecsToKeys:(const MSList *)codecs {
	...
	for (; elem != NULL; elem = elem->next) {
		...
		NSString *pref = [VirtuPhoneManager getPreferenceForCodec:pt->mime_type withRate:pt->clock_rate];
		if (pref) {
...
- (void)transformLinphoneCoreToKeys {
	...
	// root section
	{
		...
		[self setBool:[VirtuPhoneManager.instance lpConfigBoolForKey:@"auto_answer"]
...
- (void)synchronizeAccounts {
	...
	if (username && [username length] > 0 && domain && [domain length] > 0) {
		...
		if ([VirtuPhoneManager.instance lpConfigBoolForKey:@"use_phone_number" inSection:@"assistant"]) {
		...
		[VirtuPhoneManager.instance configurePushTokenForProxyConfig:proxyCfg];
...
- (BOOL)synchronize {
	@try {
		...
		[VirtuPhoneManager.instance lpConfigSetBool:enableAutoAnswer forKey:@"auto_answer"];
		...
		[VirtuPhoneManager.instance configureVbrCodecs];
		...
		[VirtuPhoneCoreSettingsStore parsePortRange:audio_port_preference minPort:&audioMinPort maxPort:&audioMaxPort];
		...
		[VirtuPhoneCoreSettingsStore parsePortRange:video_port_preference minPort:&videoMinPort maxPort:&videoMaxPort];
