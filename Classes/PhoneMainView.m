...
- (void)onGlobalStateChanged:(NSNotification *)notif {
	...
	if (state == LinphoneGlobalOn && !already_shown && VirtuPhoneManager.instance.wasRemoteProvisioned) {
		LinphoneProxyConfig *conf = linphone_core_get_default_proxy_config(LC);
		if ([VirtuPhoneManager.instance lpConfigBoolForKey:@"show_login_view" inSection:@"app"] && conf == NULL) {
			already_shown = TRUE;
...
- (void)callUpdate:(NSNotification *)notif {
	...
	switch (state) {
		...
		case LinphoneCallConnected: {
			if (![VirtuPhoneManager.instance isCTCallCenterExist]) {
				/*only register CT call center CB for connected call*/
				[VirtuPhoneManager.instance setupGSMInteraction];
...
- (void)updateApplicationBadgeNumber {
	...
	count += [VirtuPhoneManager unreadMessageCount];
...
+ (CATransition *)getBackwardTransition {
	BOOL RTL = [VirtuPhoneManager langageDirectionIsRTL];
...
- (LinphoneChatRoom *)createChatRoom:(const char *)subject addresses:(bctbx_list_t *)addresses andWaitView:(UIView *)waitView isEncrypted:(BOOL)isEncrypted isGroup:(BOOL)isGroup{
    ...
    if (!(cfg && linphone_proxy_config_get_conference_factory_uri(cfg))
        || ((bctbx_list_size(addresses) == 1) && !isGroup && ([[VirtuPhoneManager instance] lpConfigBoolForKey:@"prefer_basic_chat_room" inSection:@"misc"] || !isEncrypted))) {
