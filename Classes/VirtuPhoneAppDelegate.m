...
@implementation VirtuPhoneAppDelegate
...
- (void)applicationDidEnterBackground:(UIApplication *)application {
	...
	[VirtuPhoneManager.instance enterBackgroundMode];
...
- (void)applicationWillResignActive:(UIApplication *)application {
	...
	/* save call context */
	VirtuPhoneManager *instance = VirtuPhoneManager.instance;
...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
	...
	VirtuPhoneManager *instance = [VirtuPhoneManager instance];
	//init logs asap
	[Log enableLogs:[[VirtuPhoneManager instance] lpConfigIntForKey:@"debugenable_preference"]];
	...
	[VirtuPhoneManager.instance startLinphoneCore];
	VirtuPhoneManager.instance.iapManager.notificationCategory = @"expiry_notification";
	// initialize UI
...
- (void)applicationWillTerminate:(UIApplication *)application {
	LOGI(@"%@", NSStringFromSelector(_cmd));
	VirtuPhoneManager.instance.conf = TRUE;
	...
	if (VirtuPhoneManager.instance.pushNotificationToken != nil) {
	...
	if([CallManager incomingCallMustBeDisplayed]) {
		...
		if ([CallManager.instance.callHandled isEqualToString:callId]) {
			...
			if ([UIApplication sharedApplication].applicationState != UIApplicationStateActive && [self addLongTaskIDforCallID:callId]) {
				[VirtuPhoneManager.instance startPushLongRunningTask:loc_key callId:callId];
			}
			[VirtuPhoneManager.instance addPushCallId:callId];
		...
		NSString *uuid = [NSString stringWithFormat:@"<urn:uuid:%@>", [VirtuPhoneManager.instance lpConfigStringForKey:@"uuid" inSection:@"misc" withDefault:NULL]];
...
- (BOOL)addLongTaskIDforCallID:(NSString *)callId {
	...
	NSDictionary *dict = VirtuPhoneManager.instance.pushDict;
...
- (void)application:(UIApplication *)application
	...
	[VirtuPhoneManager.instance setPushNotificationToken:deviceToken];
}

- (void)application:(UIApplication *)application didFailToRegisterForRemoteNotificationsWithError:(NSError *)error {
	LOGI(@"%@ : %@", NSStringFromSelector(_cmd), [error localizedDescription]);
	[VirtuPhoneManager.instance setPushNotificationToken:nil];
}
...
- (void)pushRegistry:(PKPushRegistry *)registry didUpdatePushCredentials:(PKPushCredentials *)credentials forType:(PKPushType)type {
	...
	dispatch_async(dispatch_get_main_queue(), ^{
		[VirtuPhoneManager.instance setPushNotificationToken:credentials.token];
	});
}

- (void)pushRegistry:(PKPushRegistry *)registry didInvalidatePushTokenForType:(NSString *)type {
    LOGI(@"[PushKit] Token invalidated");
    dispatch_async(dispatch_get_main_queue(), ^{[VirtuPhoneManager.instance setPushNotificationToken:nil];});
}
...
- (void)userNotificationCenter:(UNUserNotificationCenter *)center
	...
	if ([response.actionIdentifier isEqual:@"Answer"]) {
		...
		LinphoneAddress *local = linphone_address_new(local_address.UTF8String);
	  	...
	  	if(room)
		  	[VirtuPhoneManager.instance send:replyText toChatRoom:room];
