...
- (BOOL)sendMessage:(NSString *)message withExterlBodyUrl:(NSURL *)externalUrl withInternalURL:(NSURL *)internalUrl {
	...
	if (internalUrl) {
        ...
        [VirtuPhoneManager setValueInMessageAppData:[internalUrl absoluteString] forKey:@"localimage" inMessage:msg];
	}
...
- (IBAction)onEncryptedDevicesClick:(id)sender {
    ...
    BOOL notAskAgain = [VirtuPhoneManager.instance lpConfigBoolForKey:@"confirmation_dialog_before_sas_call_not_ask_again"];
...
- (IBAction)onCallClick:(id)sender {
	...
	LinphoneParticipant *firstParticipant = participants ? (LinphoneParticipant *)participants->data : NULL;
	const LinphoneAddress *addr = firstParticipant ? linphone_participant_get_address(firstParticipant) : linphone_chat_room_get_peer_address(_chatRoom);
	[VirtuPhoneManager.instance call:addr];
}
...
- (NSURL *)getICloudFileUrl:(NSString *)name {
    if (@available(iOS 11.0, *)) {
        return [NSURL fileURLWithPath:[VirtuPhoneManager documentFile:name]];
    }
...
- (BOOL)writeFileInICloud:(NSData *)data fileURL:(NSURL *)fileURL {
    ...
    if ([fileManager fileExistsAtPath:[fileURL path]] || [fileName hasPrefix:@"recording"]) {
        ...
        NSString *filePath = [[VirtuPhoneManager cacheDirectory] stringByAppendingPathComponent:fileName];
        NSURL *localURL = nil;
    ...
    if ([fileManager fileExistsAtPath:filePath]) {
		...
		if ([fileType isEqualToString:@"image"] || [fileType isEqualToString:@"video"]) {
			...
			dispatch_async(dispatch_get_main_queue(), ^{
				...
				//[VirtuPhoneManager setValueInMessageAppData:filePath forKey:@"cachedfile" inMessage:message];
				[NSNotificationCenter.defaultCenter postNotificationName:kLinphoneMessageReceived object:view];
				[view.tableController scrollToLastUnread:TRUE];
			});} else {
