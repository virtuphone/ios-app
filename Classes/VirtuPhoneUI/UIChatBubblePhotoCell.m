...
- (void)setChatMessage:(LinphoneChatMessage *)amessage {
	...
	if (amessage) {
		...
		if (c) {
			...
			for (FileTransferDelegate *aftd in [VirtuPhoneManager.instance fileTransferDelegates]) {
...
- (void)update {
	...
	NSString *localImage = [VirtuPhoneManager getMessageAppDataForKey:@"localimage" inMessage:self.message];
    NSString *localVideo = [VirtuPhoneManager getMessageAppDataForKey:@"localvideo" inMessage:self.message];
    NSString *localFile = [VirtuPhoneManager getMessageAppDataForKey:@"localfile" inMessage:self.message];
...
- (void)loadFirstImage:(NSString *)key type:(PHAssetMediaType)type {
    ...
    PHFetchResult<PHAsset *> *assets = [VirtuPhoneManager getPHAssets:key];
    UIImage *img = nil;
...
- (IBAction)onFileClick:(id)sender {
    ...
    NSString *name = [VirtuPhoneManager getMessageAppDataForKey:@"localfile" inMessage:self.message];
