...
#import "VirtuPhoneAppDelegate.h"
...
static VirtuPhoneManager *theVirtuPhoneManager = nil;
...
@interface VirtuPhoneManager ()
	@property(strong, nonatomic) AVAudioPlayer *messagePlayer;
@end

@implementation VirtuPhoneManager
...
+ (VirtuPhoneManager *)instance {
	@synchronized(self) {
		if (theVirtuPhoneManager == nil) {
			theVirtuPhoneManager = [[VirtuPhoneManager alloc] init];
		}
	}
	return theVirtuPhoneManager;
}

#ifdef DEBUG
+ (void)instanceRelease {
	if (theVirtuPhoneManager != nil) {
		theVirtuPhoneManager = nil;
	}
}
#endif
...
- (id)init {
	if ((self = [super init])) {
		...
		_VirtuPhoneManagerAddressBookMap = [[OrderedDictionary alloc] init];
		pushCallIDs = [[NSMutableArray alloc] init];
		_isTesting = [VirtuPhoneManager isRunningTests];
...
- (void) setVirtuPhoneManagerAddressBookMap:(OrderedDictionary*) addressBook{
	_VirtuPhoneManagerAddressBookMap = addressBook;
}

- (OrderedDictionary*) getVirtuPhoneManagerAddressBookMap{
	return _VirtuPhoneManagerAddressBookMap;
}
...
- (void)migrationLinphoneSettings {
	...
	if ([self lpConfigBoolForKey:@"avpf_migration_done"] == FALSE) {
		...
		while (proxies) {
			...
			if (addr &&
			    strstr(addr, [VirtuPhoneManager.instance lpConfigStringForKey:@"domain_name"
					  inSection:@"app"
...
- (void)migrationPerAccount {
	...
	NSString *appDomain  = [VirtuPhoneManager.instance lpConfigStringForKey:@"domain_name"
				inSection:@"app"
				withDefault:@"sip.linphone.org"];
	while (proxies) {
...
static void migrateWizardToAssistant(const char *entry, void *user_data) {
	VirtuPhoneManager *thiz = (__bridge VirtuPhoneManager *)(user_data);
...
+ (void)dumpLcConfig {
	if (theLinphoneCore) {
		LpConfig *conf = VirtuPhoneManager.instance.configDb;
...
static void linphone_iphone_display_status(struct _LinphoneCore *lc, const char *message) {
	...
	[(__bridge VirtuPhoneManager *)linphone_core_cbs_get_user_data(linphone_core_get_current_callbacks(lc)) displayStatus:status];
}
...
static void linphone_iphone_global_state_changed(LinphoneCore *lc, LinphoneGlobalState gstate, const char *message) {
	[(__bridge VirtuPhoneManager *)linphone_core_cbs_get_user_data(linphone_core_get_current_callbacks(lc)) onGlobalStateChanged:gstate withMessage:message];
}
...
static void linphone_iphone_configuring_status_changed(LinphoneCore *lc, LinphoneConfiguringState status,
						       const char *message) {
	[(__bridge VirtuPhoneManager *)linphone_core_cbs_get_user_data(linphone_core_get_current_callbacks(lc)) onConfiguringStatusChanged:status withMessage:message];
}
...
static void linphone_iphone_registration_state(LinphoneCore *lc, LinphoneProxyConfig *cfg,
	...
	[(__bridge VirtuPhoneManager *)linphone_core_cbs_get_user_data(linphone_core_get_current_callbacks(lc)) onRegister:lc cfg:cfg state:state message:message];
}
...
static void linphone_iphone_popup_password_request(LinphoneCore *lc, LinphoneAuthInfo *auth_info, LinphoneAuthMethod method) {
	...
	if ([PhoneMainView.instance currentView] != AssistantView.compositeViewDescription) {
		...
		UIAlertAction* continueAction = [UIAlertAction actionWithTitle:NSLocalizedString(@"Confirm password", nil)
				...
				[VirtuPhoneManager.instance refreshRegisters];
			}];
...
#pragma deploymate pop
	...
	if (floor(NSFoundationVersionNumber) <= NSFoundationVersionNumber_iOS_9_x_Max) {
		...
		if ([[UIDevice.currentDevice systemVersion] floatValue] < 9 ||
		    [VirtuPhoneManager.instance lpConfigBoolForKey:@"show_msg_in_notif"] == NO) {
...
#pragma deploymate pop
			...
			if ([VirtuPhoneManager.instance lpConfigBoolForKey:@"show_msg_in_notif" withDefault:YES]) {
...
static void linphone_iphone_message_received(LinphoneCore *lc, LinphoneChatRoom *room, LinphoneChatMessage *message) {
	[(__bridge VirtuPhoneManager *)linphone_core_cbs_get_user_data(linphone_core_get_current_callbacks(lc)) onMessageReceived:lc room:room message:message];
}
...
static void linphone_iphone_message_received_unable_decrypt(LinphoneCore *lc, LinphoneChatRoom *room,
	...
	int index = [(NSNumber *)[VirtuPhoneManager.instance.pushDict objectForKey:callId] intValue] - 1;
	LOGI(@"Decrementing index of long running task for call id : %@ with index : %d", callId, index);
	[VirtuPhoneManager.instance.pushDict setValue:[NSNumber numberWithInt:index] forKey:callId];
	BOOL need_bg_task = FALSE;
	for (NSString *key in [VirtuPhoneManager.instance.pushDict allKeys]) {
		int value = [(NSNumber *)[VirtuPhoneManager.instance.pushDict objectForKey:key] intValue];
		if (value > 0) {
			need_bg_task = TRUE;
			break;
		}
	}
	if (theVirtuPhoneManager->pushBgTaskMsg && !need_bg_task) {
		LOGI(@"Message received, stopping message background task for call-id [%@]", callId);
		[[UIApplication sharedApplication] endBackgroundTask:theVirtuPhoneManager->pushBgTaskMsg];
		theVirtuPhoneManager->pushBgTaskMsg = 0;
	}
	...
	if ([UIApplication sharedApplication].applicationState != UIApplicationStateActive) {
		...
		UIAlertAction *callAction = [UIAlertAction actionWithTitle:action
					     style:UIAlertActionStyleDefault
					     handler:^(UIAlertAction *action) {
				[VirtuPhoneManager.instance call:address];
			}];
...
static void linphone_iphone_notify_received(LinphoneCore *lc, LinphoneEvent *lev, const char *notified_event,
	...
	[(__bridge VirtuPhoneManager *)linphone_core_cbs_get_user_data(linphone_core_get_current_callbacks(lc)) onNotifyReceived:lc
	 event:lev
...
static void linphone_iphone_notify_presence_received_for_uri_or_tel(LinphoneCore *lc, LinphoneFriend *lf,
	...
	[(__bridge VirtuPhoneManager *)linphone_core_cbs_get_user_data(linphone_core_get_current_callbacks(lc)) onNotifyPresenceReceivedForUriOrTel:lc
	 friend:lf
	 uri:uri_or_tel
...
static void linphone_iphone_call_encryption_changed(LinphoneCore *lc, LinphoneCall *call, bool_t on,
	...
	[(__bridge VirtuPhoneManager *)linphone_core_cbs_get_user_data(linphone_core_get_current_callbacks(lc)) onCallEncryptionChanged:lc
	 call:call
	 on:on
...
static void linphone_iphone_is_composing_received(LinphoneCore *lc, LinphoneChatRoom *room) {
	[(__bridge VirtuPhoneManager *)linphone_core_cbs_get_user_data(linphone_core_get_current_callbacks(lc)) onMessageComposeReceived:lc forRoom:room];
}
...
- (void)finishCoreConfiguration {
	...
	NSString *zrtpSecretsFileName = [VirtuPhoneManager dataFile:@"zrtp_secrets"];
	NSString *chatDBFileName = [VirtuPhoneManager dataFile:kLinphoneInternalChatDBFilename];
	...
	NSString *path = [VirtuPhoneManager bundleFile:@"nowebcamCIF.jpg"];
	if (path) {
	...
	if (![VirtuPhoneManager isNotIphone3G]) {
	...
	[NSNotificationCenter.defaultCenter postNotificationName:kLinphoneCoreUpdate
	 object:VirtuPhoneManager.instance
	 userInfo:dict];
}
...
void popup_link_account_cb(LinphoneAccountCreator *creator, LinphoneAccountCreatorStatus status, const char *resp) {
	if (status == LinphoneAccountCreatorStatusAccountLinked) {
		[VirtuPhoneManager.instance lpConfigSetInt:0 forKey:@"must_link_account_time"];
	} else {
		LinphoneProxyConfig *cfg = linphone_core_get_default_proxy_config(LC);
		if (cfg &&
		    strcmp(linphone_proxy_config_get_domain(cfg),
			   [VirtuPhoneManager.instance lpConfigStringForKey:@"domain_name"
			    inSection:@"app"
			    withDefault:@"sip.linphone.org"]
			   .UTF8String) == 0) {
			...
			[VirtuPhoneManager.instance
			 lpConfigSetInt:[[NSDate date] dateByAddingTimeInterval:[VirtuPhoneManager.instance
	...
	if (nextTime.timeIntervalSince1970 > 0 && [now earlierDate:nextTime] == nextTime) {
		...
		if (cfg) {
			...
			LinphoneAccountCreator *account_creator = linphone_account_creator_new(
											       LC,
											       [VirtuPhoneManager.instance lpConfigStringForKey:@"xmlrpc_url" inSection:@"assistant" withDefault:@""]
											       .UTF8String);
...
- (void)createLinphoneCore {
	...
	// Set audio assets
	NSString *ring =
		([VirtuPhoneManager bundleFile:[self lpConfigStringForKey:@"local_ring" inSection:@"sound"].lastPathComponent]
		 ?: [VirtuPhoneManager bundleFile:@"notes_of_the_optimistic.caf"])
		.lastPathComponent;
	NSString *ringback =
		([VirtuPhoneManager bundleFile:[self lpConfigStringForKey:@"remote_ring" inSection:@"sound"].lastPathComponent]
		 ?: [VirtuPhoneManager bundleFile:@"ringback.wav"])
		.lastPathComponent;
	NSString *hold =
		([VirtuPhoneManager bundleFile:[self lpConfigStringForKey:@"hold_music" inSection:@"sound"].lastPathComponent]
		 ?: [VirtuPhoneManager bundleFile:@"hold.mkv"])
		.lastPathComponent;
	[self lpConfigSetString:[VirtuPhoneManager bundleFile:ring] forKey:@"local_ring" inSection:@"sound"];
	[self lpConfigSetString:[VirtuPhoneManager bundleFile:ringback] forKey:@"remote_ring" inSection:@"sound"];
	[self lpConfigSetString:[VirtuPhoneManager bundleFile:hold] forKey:@"hold_music" inSection:@"sound"];
	...
	//linphone_core_set_root_ca(theLinphoneCore, [VirtuPhoneManager bundleFile:@"rootca.pem"].UTF8String);
	linphone_core_set_user_certificates_path(theLinphoneCore, [VirtuPhoneManager cacheDirectory].UTF8String);
...
- (void)playMessageSound {
	...
	AudioServicesPlaySystemSound(VirtuPhoneManager.instance.sounds.vibrate);
}
...
- (void)startPushLongRunningTask:(NSString *)loc_key callId:(NSString *)callId {
	...
	if ([loc_key isEqualToString:@"IM_MSG"]) {
		...
		pushBgTaskMsg = [[UIApplication sharedApplication] beginBackgroundTaskWithExpirationHandler:^{
				...
				for (NSString *key in [VirtuPhoneManager.instance.pushDict allKeys]) {
					[VirtuPhoneManager.instance.pushDict setValue:[NSNumber numberWithInt:0] forKey:key];
				}
				[[UIApplication sharedApplication] endBackgroundTask:pushBgTaskMsg];
				pushBgTaskMsg = 0;
			}];
...
- (void)enableProxyPublish:(BOOL)enabled {
	...
	linphone_core_enable_friend_list_subscription(LC, enabled && [VirtuPhoneManager.instance lpConfigBoolForKey:@"use_rls_presence"]);
}
...
- (BOOL)enterBackgroundMode {
	...
	if (proxyCfg) {
		...
		if ([VirtuPhoneManager.instance lpConfigBoolForKey:@"backgroundmode_preference"] || pushNotifEnabled) {
			if (floor(NSFoundationVersionNumber) <= NSFoundationVersionNumber_iOS_9_x_Max) {
		...
		if ([VirtuPhoneManager.instance lpConfigBoolForKey:@"voip_mode_preference"] && [VirtuPhoneManager.instance lpConfigBoolForKey:@"backgroundmode_preference"] && !pushNotifEnabled) {
...
- (void)migrateImportantFiles {
    if ([VirtuPhoneManager copyFile:[VirtuPhoneManager documentFile:@"linphonerc"] destination:[VirtuPhoneManager preferenceFile:@"linphonerc"] override:TRUE ignore:TRUE])
        [NSFileManager.defaultManager
         removeItemAtPath:[VirtuPhoneManager documentFile:@"linphonerc"]
         error:nil];

    if ([VirtuPhoneManager copyFile:[VirtuPhoneManager documentFile:@"linphone_chats.db"] destination:[VirtuPhoneManager dataFile:@"linphone_chats.db"] override:TRUE ignore:TRUE])
        [NSFileManager.defaultManager
         removeItemAtPath:[VirtuPhoneManager documentFile:@"linphone_chats.db"]
         error:nil];

    if ([VirtuPhoneManager copyFile:[VirtuPhoneManager documentFile:@"zrtp_secrets"] destination:[VirtuPhoneManager dataFile:@"zrtp_secrets"] override:TRUE ignore:TRUE])
        [NSFileManager.defaultManager
         removeItemAtPath:[VirtuPhoneManager documentFile:@"zrtp_secrets"]
         error:nil];

    if ([VirtuPhoneManager copyFile:[VirtuPhoneManager documentFile:@"zrtp_secrets.bkp"] destination:[VirtuPhoneManager dataFile:@"zrtp_secrets.bkp"] override:TRUE ignore:TRUE])
        [NSFileManager.defaultManager
         removeItemAtPath:[VirtuPhoneManager documentFile:@"zrtp_secrets.bkp"]
         error:nil];
}
...
- (void)renameDefaultSettings {
	...
	NSString *src = [VirtuPhoneManager documentFile:@".linphonerc"];
	NSString *dst = [VirtuPhoneManager preferenceFile:@"linphonerc"];
...
- (void)copyDefaultSettings {
	NSString *src = [VirtuPhoneManager bundleFile:@"linphonerc"];
	NSString *srcIpad = [VirtuPhoneManager bundleFile:@"linphonerc~ipad"];
	if (IPAD && [[NSFileManager defaultManager] fileExistsAtPath:srcIpad]) {
		src = srcIpad;
	}
	NSString *dst = [VirtuPhoneManager preferenceFile:@"linphonerc"];
	[VirtuPhoneManager copyFile:src destination:dst override:FALSE ignore:FALSE];
}

- (void)overrideDefaultSettings {
	NSString *factory = [VirtuPhoneManager bundleFile:@"linphonerc-factory"];
	NSString *factoryIpad = [VirtuPhoneManager bundleFile:@"linphonerc-factory~ipad"];
	if (IPAD && [[NSFileManager defaultManager] fileExistsAtPath:factoryIpad]) {
		factory = factoryIpad;
	}
	NSString *confiFileName = [VirtuPhoneManager preferenceFile:@"linphonerc"];
...
- (void)setupGSMInteraction {
	...
	__block __weak VirtuPhoneManager *weakSelf = self;
