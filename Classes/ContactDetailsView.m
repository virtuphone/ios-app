...
- (void)removeContact {
	inhibUpdate = TRUE;
	[[VirtuPhoneManager.instance fastAddressBook] deleteContact:_contact];
	inhibUpdate = FALSE;
...
- (void)saveData {
    ...
    for (NSString *sipAddr in _contact.sipAddresses) {
        ...
        [VirtuPhoneManager.instance.fastAddressBook saveContact:_contact];
}
...
- (void)modifyTmpContact:(Contact *)acontact {
	...
	if (!acontact) {
        ...
        @synchronized(VirtuPhoneManager.instance.fastAddressBook) {
          _tmpContact = [[Contact alloc]
              initWithCNContact:[VirtuPhoneManager.instance.fastAddressBook
...
- (void)addCurrentContactContactField:(NSString *)address {
	...
	if (([username rangeOfString:@"@"].length > 0) &&
		([VirtuPhoneManager.instance lpConfigBoolForKey:@"show_contacts_emails_preference"] == true)) {
		[_tableController addEmailField:username];
	} else if ((linphone_proxy_config_is_phone_number(NULL, [username UTF8String])) &&
			   ([VirtuPhoneManager.instance lpConfigBoolForKey:@"save_new_contacts_as_phone_number"] == true)) {
...
- (void)newContact:(NSString *)address {
  ...
  [VirtuPhoneManager.instance becomeActive];
}
...
- (IBAction)onEditClick:(id)event {
	if (_tableController.isEditing) {
		[VirtuPhoneManager.instance setContactsUpdated:TRUE];
		[self setEditing:FALSE];
...
- (BOOL) hasDuplicateContactOf:(Contact*) contactToCheck{
	...
	if(mCNContact == NULL){
		for(NSString *address in contactToCheck.sipAddresses){
			...
			if([VirtuPhoneManager.instance.fastAddressBook.addressBookMap objectForKey:name]){
				return TRUE;
			}
		}
		return FALSE;
