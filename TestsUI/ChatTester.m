...
- (void)afterAll {
	...
	[self goBackFromChat];
	ASSERT_EQ([VirtuPhoneManager instance].fileTransferDelegates.count, 0)
}
...
- (void)downloadImageWithQuality:(NSString *)quality {
	...
	for (int i = 0; i < 180; i++) {
		...
		if (VirtuPhoneManager.instance.fileTransferDelegates.count == 0)
			break;
	}
	...
	ASSERT_EQ(VirtuPhoneManager.instance.fileTransferDelegates.count, 1);
}
...
- (void)testPerformanceHugeChatList {
	...
	LinphoneCore *lc = [VirtuPhoneManager getLc];
	for (int i = 0; i < 100; i++) {
...
- (void)testRemoveAllChats {
	...
	ASSERT_EQ(linphone_core_get_chat_rooms([VirtuPhoneManager getLc]), NULL);
}
...
// wait for bugfix
	...
	timeout = 30;
	while ([VirtuPhoneManager instance].fileTransferDelegates.count > 0 && timeout) {
		[tester waitForTimeInterval:.5];
		timeout--;
	}
	[self goBackFromChat];
